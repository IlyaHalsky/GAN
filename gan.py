...
def process_data():
    ...
    pokemon_dir = os.path.join(current_dir, 'resize_black')
    images = []
...
def train():
    random_dim = 100
    print("test", os.environ['CUDA_VISIBLE_DEVICES'])
    ...
    # continue training
    #save_path = saver.save(sess, './model/' + version + '/' + str(0))
    ...
    test_noise = np.random.uniform(-1.0, 1.0, size=[batch_size, random_dim]).astype(np.float32)

    print('total training sample num:%d' % samples_num)
    print('batch size: %d, batch num per epoch: %d, epoch num: %d' % (batch_size, batch_num, EPOCH))
    print('start training...')
    for i in range(startI, EPOCH):
        ...
        if i % 10 == 0:
            ...
            imgtest = sess.run(fake_image, feed_dict={random_input: test_noise, is_train: False})
